...
import type { AdAnalytics, AdImpression, AdClick } from '@prisma/client';
...
export interface ImpressionResult extends AdImpression {
  success: boolean;
  fraudResult?: FraudDetectionResult;
}

export interface ClickResult extends AdClick {
  success: boolean;
  fraudResult?: FraudDetectionResult;
}
...
export class AdAnalyticsService {
  ...
  async recordImpression(event: ImpressionEvent): Promise<ImpressionResult> {
    try {
      ...
      if (!advertisement || advertisement.campaign.status !== 'active') {
        ...
        return {
          ...({} as AdImpression), // Empty impression data
          success: false,
          fraudResult: {
            isValid: false,
            riskScore: 0,
      ...
      const impression = await this.prisma.adImpression.create({
        data: {
          ...
          isViewable: event.isViewable ?? true,
          cost: event.cost,
        },
      });
        ...
        try {
          await elasticsearch.index({
            ...
            document: {
              ...
              cost: event.cost,
              timestamp: impression.createdAt,
            },
          });
        } catch (esError) {
  ...
  async getPlatformAnalytics(options: {
    ...
    try {
      ...
      // Get platform breakdown from impressions
      const platformBreakdown = await this.prisma.adImpression.groupBy({
        by: ['platform'],
        where: {
          createdAt: {
            gte: options.startDate,
