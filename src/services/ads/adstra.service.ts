...
export class AdstraService {
  ...
  constructor(config: AdstraConfig) {
    ...
    this.client = axios.create({
      ...
      headers: {
        ...
        'User-Agent': 'Vikareta-Adstra-Integration/1.0'
      }
    });
  ...
  private setupInterceptors(): void {
    this.client.interceptors.request.use(
      (config) => {
        logger.debug('Adstra API Request', {
          url: config.url,
      ...
      (error) => {
        logger.error('Adstra API Request Error', { error: error.message });
    ...
    this.client.interceptors.response.use(
      (response) => {
        ...
        logger.debug('Adstra API Response', {
  ...
  async requestAd(request: AdstraAdRequest): Promise<AdstraAd | null> {
    try {
      if (!this.isHealthy) {
        logger.warn('Adstra service is unhealthy, skipping ad request');
        return null;
      }
      ...
      if (!response.data || !response.data.ad) {
        logger.info('No Adstra ad available for request', { request });
        return null;
      }
      ...
      if (error instanceof AxiosError) {
        if (error.response?.status === 404) {
          logger.info('No Adstra ad available', { request });
          return null;
        }
        
        if (error.response?.status === 429) {
          logger.warn('Adstra rate limit exceeded', { request });
          return null;
        }
      }

      // Check if it's an axios-like error (for testing compatibility)
      ...
      logger.error('Failed to request Adstra ad', {
  ...
  async trackRevenue(revenueData: AdstraRevenueData): Promise<void> {
    try {
      ...
      logger.info('Adstra revenue tracked successfully', { revenueData });

    } catch (error) {
      logger.error('Failed to track Adstra revenue', {
        error: error instanceof Error ? error.message : error,
        revenueData
      });
  ...
  async syncPerformanceData(): Promise<void> {
    try {
      ...
      logger.error('Failed to sync Adstra performance data', {
        error: error instanceof Error ? error.message : error
      });
    }
  }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.debug('Adstra health check completed', {
      ...
      logger.error('Adstra health check failed', {
        error: error instanceof Error ? error.message : error
      });
  ...
  getServiceStatus(): {
    ...
    config: Omit<AdstraConfig, 'apiKey'>;
  } {
    return {
      healthy: this.isHealthy,
...
// Factory function to create Adstra service instance
export function createAdstraService(): AdstraService {
  const config: AdstraConfig = {
    publisherId: process.env['ADSTRA_PUBLISHER_ID'] || '',
    apiKey: process.env['ADSTRA_API_KEY'] || '',
