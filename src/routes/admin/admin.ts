...
router.get('/dashboard/content-stats', authenticate, requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  try {
    const [
      ...
      prisma.product.count({ where: { status: 'UNVERIFIED' } }),
...
router.get('/dashboard/disputes', authenticate, requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const disputes = disputeOrders.map(order => ({
      id: order.id,
      // Field removed
      type: order.status === 'CANCELLED' ? 'cancellation' : order.status === 'REFUNDED' ? 'return' : 'dispute',
      status: 'PENDING',
      amount: order.totalAmount,
    ...
    const features = [
      {
        ...
        enabled: true,
        productCategory: 'user_management',
      },
      {
        id: 'product_listing',
